...
public abstract class AbstractIME extends InputMethodService implements
	...
	public void onCreate() {
		...
		txtReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context arg0, Intent arg1) {
				...
				// We can't commitText() on this point. Maybe because the
				// InputConnection is invalid now.
				// Instead, we store it temporarily. Later on onBindInput() we
				// can commit it.
			}
		};
	...
	public void onKey(int primaryCode, int[] keyCodes) {
		...
		if (handleOption(primaryCode) || handleCapsLock(primaryCode)
				...
				|| handleDelete(primaryCode) || handleDPAD(primaryCode)
				|| handleComposing(primaryCode)) {
			return;
		}
							...
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface di,
									...
									case MENU_BARCODESCAN: // Scan Barcode
										...
										iBCScan.setClass(AbstractIME.this,
									...
									case MENU_SETTINGS: // Settings
										...
										iSetting.setClass(AbstractIME.this,
												ImePreferenceActivity.class);
										iSetting.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
	 ...
	 * 
	 * @param sKB
	 *            The soft keyboard object. To check if it is ready.
	...
	/**
	 * Handles the Language Change event (English <-> Chinese). Normally Shift +
	 * Space key, or Language Change key
	 * 
	 * @param keyCode
	 * @param event
	...
	public boolean handleLanguageChange(int keyCode, KeyEvent event) {
		if ((event.isShiftPressed() && keyCode == KeyEvent.KEYCODE_SPACE)
				|| keyCode == 1000 || keyCode == 64) { // 1000 is hard-coded by
														// MS3
